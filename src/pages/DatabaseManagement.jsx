...
import { Save, RotateCcw, Upload, ShieldAlert, Trash2 } from 'lucide-react';
import { APP_ENVIRONMENT, IS_PRODUCTION, RESET_CONFIRM_PHRASE, RESTORE_CONFIRM_PHRASE } from '@/lib/dbEnv';
...
export default function DatabaseManagement() {
  ...
  const doSave = async () => {
    setBusy(true);
    try {
      const res = await base44.functions.invoke('databaseBackup', { name: bkName, notes: bkNotes });
      toast({ title: 'Backup berhasil', description: res.data?.backup?.backup_code });
      setSaveOpen(false); setBkName(''); setBkNotes('');
      loadBackups();
    } catch (e) {
  ...
  const doReset = async () => {
    ...
    try {
      const res = await base44.functions.invoke('databaseReset', {
        mode: resetMode, resetSequences, skipBackup, confirm: resetPhrase,
      });
  ...
  const doRestore = async () => {
    ...
    try {
      const res = await base44.functions.invoke('databaseRestore', {
        backup_id: restoreId, mode: restoreMode, confirm: restorePhrase, autoBackup: true,
      });
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      ...
      {IS_PRODUCTION && (
        <div className="mb-4 bg-red-50 border border-red-300 rounded-lg px-4 py-3 text-[13px] text-red-700 flex items-center gap-2">
          <ShieldAlert className="w-4 h-4 shrink-0" /> Reset database dinonaktifkan pada environment Production.
        </div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white border rounded-lg p-4 flex flex-col">
          <div className="flex items-center gap-2 mb-2"><Save className="w-4 h-4 text-primary" /><h3 className="font-semibold text-[14px]">Save Database</h3></div>
          <p className="text-[12px] text-muted-foreground mb-3 flex-1">Buat backup seluruh database sebelum perubahan besar.</p>
          <Button onClick={() => setSaveOpen(true)} className="w-full"><Save className="w-4 h-4" /> Save Database</Button>
        </div>
        <div className="bg-white border rounded-lg p-4 flex flex-col">
          <div className="flex items-center gap-2 mb-2"><Upload className="w-4 h-4 text-primary" /><h3 className="font-semibold text-[14px]">Restore Database</h3></div>
          <p className="text-[12px] text-muted-foreground mb-3 flex-1">Kembalikan database dari backup yang tersedia.</p>
          <Button variant="outline" onClick={() => setRestoreOpen(true)} className="w-full"><Upload className="w-4 h-4" /> Lihat Backup</Button>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[12.5px]">
            ...
            <tbody>
              ...
              {!loading && backups.length === 0 && <tr><td colSpan={8} className="px-3 py-6 text-center text-muted-foreground">Belum ada backup</td></tr>}
              {!loading && backups.map((b) => (
                <tr key={b.id} className="border-t hover:bg-muted/30">
                  <td className="px-3 py-2 font-mono">{b.backup_code}</td>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex gap-1">
                      <Button size="sm" variant="ghost" disabled={b.status !== 'COMPLETED'} onClick={() => { setRestoreId(b.id); setRestoreOpen(true); }} title="Restore"><Upload className="w-3.5 h-3.5" /></Button>
      ...
      <FormModal open={saveOpen} onClose={() => setSaveOpen(false)} title="Save Database" submitLabel="Buat Backup" submitting={busy} onSubmit={doSave} size="md">
        <div className="space-y-3">
          ...
          <p className="text-[11px] text-muted-foreground">Backup mencakup master, resep, transaksi, stok, HPP, batch. Disimpan di private storage dengan checksum SHA-256.</p>
        </div>
      </FormModal>
      ...
      <FormModal open={restoreOpen} onClose={() => setRestoreOpen(false)} title="Restore Database" submitLabel="Jalankan Restore" submitting={busy} onSubmit={doRestore} size="md">
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              <button type="button" onClick={() => setRestoreMode('operational')} className={`border rounded px-3 py-2 text-left text-[12px] ${restoreMode === 'operational' ? 'border-primary bg-primary/5' : 'border-input'}`}>
                <div className="font-semibold">Operational</div>
                <div className="text-muted-foreground text-[11px]">Master+resep+transaksi+stok. User tetap.</div>
              </button>
              <button type="button" disabled className="border rounded px-3 py-2 text-left text-[12px] opacity-50 cursor-not-allowed">
                <div className="font-semibold">Full (termasuk user)</div>
                <div className="text-muted-foreground text-[11px]">Tidak didukung (user via invite/auth).</div>
              </button>
            </div>
          </div>
