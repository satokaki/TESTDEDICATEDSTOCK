...
export default function Excise() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const sm = {};
      matBal.forEach(b => {
        sm[b.item_id] = (sm[b.item_id] || 0) + (b.available_quantity || 0);
      });
      setExciseStocks(sm);
    } catch {
  ...
  const exciseTotalRequired =
    (Number(form.quantity) || 0) * (Number(form.excise_quantity_per_unit) || 0);

  const boxTotalRequired =
    (Number(form.quantity) || 0) * (Number(form.box_quantity_per_unit) || 0);
  ...
  // Cukai hanya meneruskan identitas produk hasil Labeling.
  // Tidak ada remap brand/product di tahap ini.
  ...
  const handleSubmit = async () => {
    if (!form.product_id || !form.quantity || !form.operator) {
      toast({ variant: 'destructive', title: 'Produk, jumlah, dan operator wajib diisi' });
      return;
    }

    const stockItem = belumCukaiStock.find(s => s.id === form.stock_id);
    if (!stockItem) {
      toast({ variant: 'destructive', title: 'Stok belum cukai tidak ditemukan' });
      return;
    }

    if (Number(form.quantity) > Number(stockItem.available_quantity)) {
      toast({
        variant: 'destructive',
    ...
    if (form.excise_material_id) {
      ...
      if (exciseTotalRequired > stk) {
        toast({
          ...
          description: `Butuh ${exciseTotalRequired}, stok ${stk}`,
        });
        return;
      }
    }
    ...
    if (form.use_box && form.box_material_id) {
      ...
      if (boxTotalRequired > stk) {
        toast({
          ...
          description: `Butuh ${boxTotalRequired}, stok ${stk}`,
        });
        return;
      }
    }
    ...
    try {
      ...
      if (!product) throw new Error('Produk hasil Labeling tidak ditemukan');

      // Guard identitas: produk form WAJIB sama dengan item_id stok UNEXCISED.
      if (form.product_id !== stockItem.item_id) {
        throw new Error('Identitas produk berubah setelah Labeling. Proses cukai dihentikan.');
      }

      const brand = brands.find(b => b.id === product.brand_id);
      const excNumber = await generateOrderNumber('EXC', 'ExciseOrder');

      // HPP wajib dibaca dari labeling_output produk HASIL Labeling.
      ...
      if (!labelingLedgers.length || hppLabelingPerBottle <= 0) {
        throw new Error(
      ...
      const excise = await base44.entities.ExciseOrder.create({
        ...
        brand_name: brand?.name || product.brand_name || '',
        product_id: product.id,
        product_name: product.name || '',
        batch_number: stockItem.batch_number || '',
        bottle_size: Number(product.bottle_size || form.bottle_size),
        quantity: Number(form.quantity),
        excise_label_type: form.excise_label_type,
        document_number: form.document_number,
        excise_reference_number: form.excise_reference_number,
        excise_material_id: form.excise_material_id || '',
        excise_material_name: form.excise_material_name || '',
        excise_quantity_per_unit: Number(form.excise_quantity_per_unit) || 1,
        excise_total_required: form.excise_material_id ? exciseTotalRequired : 0,
      ...
      // OUT produk yang sama dari UNEXCISED.
      ...
      if (form.excise_material_id) {
        ...
        await recordStockMovement({
          ...
          item_name: form.excise_material_name,
          item_code: mat?.code || '',
          inventory_status: '',
          quantity_out: exciseTotalRequired,
          unit: mat?.unit || 'unit',
          unit_cost: exciseHbt,
          transaction_type: 'excise_consumption',
          transaction_number: excNumber,
          reference_type: 'excise',
          reference_id: excise.id,
          notes: `Pita cukai untuk ${excNumber}`,
        });
      }
      ...
      if (form.use_box && form.box_material_id) {
        ...
        await recordStockMovement({
          ...
          item_name: form.box_material_name,
          item_code: boxMat?.code || '',
          inventory_status: '',
          quantity_out: boxTotalRequired,
          unit: boxMat?.unit || 'pcs',
          unit_cost: packagingHbt,
          transaction_type: 'excise_consumption',
          transaction_number: excNumber,
          reference_type: 'excise',
          reference_id: excise.id,
          notes: `Box kemasan untuk ${excNumber}`,
        });
      }

      const totalFinalCost = previousProductCost + exciseCost + packagingCost;
      const hppFinalPerBottle =
        quantityProcessed > 0 ? totalFinalCost / quantityProcessed : 0;
      const safeHppFinal =
        Number.isFinite(hppFinalPerBottle) ? hppFinalPerBottle : 0;

      // IN READY_FOR_SALE tetap identitas produk hasil Labeling.
      await recordStockMovement({
        item_type: 'product',
        item_id: product.id,
        item_name: product.name || '',
      ...
      toast({
        title: 'Proses cukai selesai',
        description: `${excNumber} · ${product.name}`,
      });
  ...
  const exportExcisePDF = async (row) => {
    try {
      exportDocumentToPDF({
        ...
        itemRows: [{
          desc: `Proses cukai ${row.quantity} unit ${row.product_name} (batch ${row.batch_number || '-'}) — ref ${row.excise_reference_number || '-'}`,
        }],
  ...
  return (
    <div className="p-5 max-w-[1400px] mx-auto">
      <PageHeader
        ...
        actions={
          <Button onClick={openAdd} size="sm" className="gap-1.5">
            <Plus className="w-4 h-4" /> Proses Cukai Baru
          </Button>
        }
      />
      ...
      <DataTable
        ...
        searchKeys={['excise_number', 'product_name', 'batch_number']}
      ...
      >
          ...
          <Select value={form.stock_id} onValueChange={onStockChange}>
            ...
            <SelectContent>
              {belumCukaiStock.map(s => {
                const p = products.find(x => x.id === s.item_id);
                return (
                  <SelectItem key={s.id} value={s.id}>
                    {getInventoryDisplayName(p?.name || s.item_name, 'UNEXCISED')}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-[12.5px] mb-1">Produk</Label>
            <Input
              value={products.find(p => p.id === form.product_id)?.name || ''}
              disabled
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Ukuran Botol (ml)</Label>
            <NumberInput
              value={form.bottle_size}
              onChange={v => setForm(f => ({ ...f, bottle_size: v }))}
              allowDecimal
              maxDecimals={1}
              min={0}
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Jumlah *</Label>
            <NumberInput
              value={form.quantity}
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Tanggal Proses</Label>
            <Input
              type="date"
              value={form.excise_date}
              onChange={e => setForm(f => ({ ...f, excise_date: e.target.value }))}
              className="h-9 text-[13px]"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <Select value={form.excise_material_id} onValueChange={onCukaiChange}>
            <SelectTrigger className="h-9 text-[13px]">
              <SelectValue placeholder="Pilih pita cukai dari stok" />
            </SelectTrigger>
            <SelectContent>
              {exciseMaterials.map(m => {
                const stk = exciseStocks[m.id] || 0;
                return (
                  <SelectItem key={m.id} value={m.id} disabled={stk <= 0}>
          ...
          <div className="flex items-center gap-2 mb-2">
            <Switch
              ...
              onCheckedChange={v => setForm(f => ({
                ...f,
                use_box: v,
                box_material_id: v ? f.box_material_id : '',
                box_material_name: v ? f.box_material_name : '',
              }))}
            />
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                ...
                <Select
                  ...
                  onValueChange={v => {
                    const m = boxMaterials.find(x => x.id === v);
                    setForm(f => ({
                      ...f,
                      box_material_id: v,
                      box_material_name: m?.name || '',
                    }));
                  }}
                >
                  <SelectTrigger className="h-9 text-[13px]">
                    <SelectValue placeholder="Pilih box dari stok" />
                  </SelectTrigger>
                  <SelectContent>
                    {boxMaterials.map(m => {
                      const stk = exciseStocks[m.id] || 0;
              ...
              <div>
                <Label className="text-[12.5px] mb-1">Per Unit</Label>
                <NumberInput
                  value={form.box_quantity_per_unit}
                  onChange={v => setForm(f => ({ ...f, box_quantity_per_unit: v }))}
                  allowDecimal
                  maxDecimals={4}
                  min={0}
                  className="h-9 text-[13px]"
              ...
              <div className="col-span-3 text-[11.5px] text-muted-foreground">
                ...
                <span className="font-semibold tabular-nums">
                  {form.box_material_id ? boxTotalRequired : '—'}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            <Label className="text-[12.5px] mb-1">Per Unit</Label>
            <NumberInput
              value={form.excise_quantity_per_unit}
              onChange={v => setForm(f => ({ ...f, excise_quantity_per_unit: v }))}
              allowDecimal
              min={0}
              className="h-9 text-[13px]"
            />
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Total Butuh</Label>
            <Input
              value={form.excise_material_id ? (exciseTotalRequired || '') : '—'}
              disabled
              className="h-9 text-[13px] bg-muted/40"
            />
          </div>
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Jenis Pita (label)</Label>
            <Input
              value={form.excise_label_type}
              onChange={e => setForm(f => ({ ...f, excise_label_type: e.target.value }))}
              className="h-9 text-[13px]"
            />
          </div>

          <div>
            <Label className="text-[12.5px] mb-1">Nomor Dokumen</Label>
            <Input
          ...
          <div>
            <Label className="text-[12.5px] mb-1">Nomor Referensi Cukai</Label>
            <Input
              value={form.excise_reference_number}
              onChange={e => setForm(f => ({ ...f, excise_reference_number: e.target.value }))}
              className="h-9 text-[13px]"
            />
          </div>
